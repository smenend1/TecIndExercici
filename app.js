...
// v12: resolucions docents pas a pas per al banc PAU.
// Abast d'aquesta versió: Tecnologia i Enginyeria 2025 i 2024 amb resolució completa o guiada detallada.
// Els exercicis amb figura complexa inclouen procediment de resolució i, quan no es pot verificar numèricament sense la figura, queden marcats per revisar visualment.
const pauSolutionsV12 = {
  "full-tec2025-s1-e1": [
    {title:"1. Qüestions resoltes de manera guiada", prompt:"Resol les qüestions seleccionades i justifica l'opció triada.", hint:"Comprova sempre unitats i si el valor demanat és màxim, eficaç, energia o potència.", solution:"Q1: 10 mm − 0,05 mm = 9,95 mm → opció b.\nQ2: motor de 4 temps: 2 voltes per cicle. 2500 min⁻¹ → 1250 cicles/min → 75000 cicles/h. 7,5 kg/h / 75000 · 100 = 0,010 kg = 10 g → opció b.\nQ3: sense pèrdues, l'energia depèn del desnivell, no del camí. Si els punts inicial i final són els mateixos, l'energia és la mateixa → opció a.\nQ4: Ief = U/R = 230/17 = 13,53 A. Imax = Ief·√2 = 19,13 A → opció b.\nQ6: Q = m·ce·ΔT = 40·0,385·(250−25) = 3465 kJ → opció d.\nQ8: XL = 2πfL = 31,42 Ω. |Z| = U/I = 115 Ω. R = √(115²−31,42²) = 110,6 Ω → opció b.", keywords:["9,95","10 g","mateixa","19,13","3465","111"]},
    {title:"2. Qüestions amb figura o taula", prompt:"Identifica quines qüestions necessiten mirar la figura original.", hint:"L'assaig de tracció i l'ajust H8/f6 depenen de la figura o de dades tabulades.", solution:"Q5 necessita interpretar la corba tensió-deformació de la figura. Cal mirar quin acer abandona la zona elàstica a una tensió més alta.\nQ7 necessita els valors de desviació/tolerància de l'ajust 200 H8/f6 que apareixen en la prova. Sense aquests valors visibles, no s'ha de donar una resposta numèrica a cegues.", keywords:["figura","corba","ajust","H8","f6"]}
  ],
  "full-tec2025-s1-e2": [
    {title:"1. Interpretar les condicions", prompt:"Tradueix l'enunciat a condicions lògiques.", hint:"'Excepte si el mode nocturn està activat' vol dir que cal ¬n.", solution:"La persiana puja en dos casos: (1) hi ha llum exterior, la temperatura no és alta i el mode nocturn no està activat; (2) hi ha persones i la temperatura és alta, independentment de la llum i del mode nocturn.", keywords:["llum","temperatura","nocturn","persones"]},
    {title:"2. Funció lògica", prompt:"Escriu la funció lògica simplificada.", hint:"Uneix els dos casos amb una OR.", solution:"p = l·¬t·¬n + i·t.\nEl primer producte representa el cas de llum exterior amb temperatura baixa i mode nocturn desactivat. El segon producte representa presència de persones amb temperatura alta.", keywords:["l","t","n","i","or","and"]},
    {title:"3. Taula i portes", prompt:"Explica com fer la taula de veritat i el circuit.", hint:"Hi ha 4 entrades, per tant 16 combinacions.", solution:"La taula de veritat té 16 files. A cada fila es calcula primer ¬t i ¬n, després l·¬t·¬n i i·t, i finalment la sortida p amb una OR. El circuit equivalent necessita dues portes NOT, dues AND i una OR final.", keywords:["16","NOT","AND","OR"]}
  ],
  "full-tec2025-s1-e3": [
    {title:"a) Potència mecànica", prompt:"Calcula la potència mecànica necessària per elevar la massa d'una columna.", hint:"Primer calcula energia potencial i després divideix pel temps.", solution:"Dades: m = 1900 kg, h = 1,8 m, t = 45 s.\nE = m·g·h = 1900·9,81·1,8 = 33550 J.\nPmec = E/t = 33550/45 = 745,6 W.\nResultat: Pmec ≈ 0,746 kW per columna.", keywords:["745","0,746","mgh","potència"]},
    {title:"b) Potència elèctrica", prompt:"Aplica el rendiment del sistema.", hint:"Si el rendiment és menor que 1, la potència elèctrica ha de ser més gran que la mecànica.", solution:"η = 0,4.\nPelèc = Pmec/η = 745,6/0,4 = 1864 W.\nResultat: cada motor consumirà aproximadament 1,86 kW.", keywords:["1864","1,86","rendiment"]},
    {title:"c) Voltes del cargol", prompt:"Calcula quantes voltes cal fer per pujar 1,8 m si el pas és de 7 mm/volta.", hint:"Converteix 7 mm a 0,007 m.", solution:"pas = 7 mm/volta = 0,007 m/volta.\nnvoltes = h/pas = 1,8/0,007 = 257,1 voltes.\nResultat: calen aproximadament 257 voltes del cargol.", keywords:["257","0,007","voltes"]},
    {title:"d) Relació de transmissió", prompt:"Determina la reducció necessària entre motor i cargol.", hint:"Primer calcula la velocitat del cargol en min⁻¹.", solution:"El cargol fa 257,1 voltes en 45 s = 0,75 min.\nncargol = 257,1/0,75 = 342,9 min⁻¹.\nMotor: nmotor = 1420 min⁻¹.\nRelació reductora i = nmotor/ncargol = 1420/342,9 = 4,14.\nEquivalentment, τ = ncargol/nmotor = 0,241.\nResultat: cal un reductor aproximat de 4,14:1.", keywords:["342","4,14","0,241","reductor"]}
  ],
  "full-tec2025-s1-e4": [
    {title:"a) Gas per començar a bullir", prompt:"Calcula l'energia útil per escalfar l'aigua fins a 100 °C i la massa de gas equivalent.", hint:"2 L d'aigua equivalen aproximadament a 2 kg.", solution:"m = 2 kg, ΔT = 100−15 = 85 °C, ce = 4,186 kJ/(kg·°C).\nQútil = m·ce·ΔT = 2·4,186·85 = 711,6 kJ.\nCom η = 0,2, Egas = Qútil/η = 711,6/0,2 = 3558 kJ.\npc = 45,8 MJ/kg = 45800 kJ/kg.\nmgas1 = 3558/45800 = 0,0777 kg = 77,7 g.", keywords:["711","3558","77,7"]},
    {title:"b) Temps fins a ebullició", prompt:"Calcula el temps si el consum màxim és 155 g/h.", hint:"Fes servir la massa de gas trobada a l'apartat anterior.", solution:"t = mgas/consum = 77,7 g / 155 g/h = 0,501 h.\n0,501 h · 60 = 30,1 min.\nResultat: tarda aproximadament 30 minuts a començar a bullir.", keywords:["30","0,501","min"]},
    {title:"c) Consum total i percentatge del cartutx", prompt:"Afegeix el gas gastat durant la cocció posterior.", hint:"El consum durant la cocció és el 30% del consum màxim durant 18 min.", solution:"Consum durant la cocció: 0,30·155 g/h·(18/60 h) = 13,95 g.\nGas total = 77,7 + 13,95 = 91,65 g.\nPercentatge del cartutx = 91,65/230·100 = 39,8 %.\nResultat: es consumeix aproximadament el 40 % del cartutx.", keywords:["13,95","91,65","39,8","40"]}
  ],
  "full-tec2024-s1-e1": [
    {title:"Qüestions amb càlcul directe", prompt:"Resol les preguntes numèriques principals del test.", hint:"Vigila cicles de motor, massa d'aigua, kWh i volum de cambra.", solution:"Q1: consum = 10,2 L/h·0,8 kg/L = 8,16 kg/h. A 4000 min⁻¹ i motor 4T hi ha 2000 cicles/min = 120000 cicles/h. Massa per cicle = 8,16/120000 = 0,000068 kg = 68 mg → opció c.\nQ3: 50 ml d'aigua ≈ 0,05 kg. ΔT = Q/(m·ce) = 3 kJ/(0,05·4,18) = 14,35 K. Tfinal = 5 + 14,35 = 19,35 °C → opció d.\nQ4: E = irradiació·àrea·rendiment·dies = 13·4,4·0,3·30 = 514,8 kWh → opció a.\nQ5: Vcil = π·d²/4·cursa = π·5,6²/4·5,06 = 124,6 cm³. r = (Vcil+Vc)/Vc = 7,3 → Vc = Vcil/(7,3−1)=19,8 cm³ → opció d.", keywords:["68","19,35","514,8","19,8"]},
    {title:"Qüestions amb figura", prompt:"Explica com resoldre la pregunta de l'assaig de tracció.", hint:"El mòdul d'elasticitat és la pendent inicial de la corba tensió-deformació.", solution:"Q2 depèn de la figura: cal comparar la pendent del tram elàstic inicial de cada acer. La corba amb pendent inicial més gran té mòdul d'elasticitat més alt. No s'ha de confondre límit elàstic amb mòdul d'elasticitat.", keywords:["pendent","mòdul","elasticitat"]}
  ],
  "full-tec2024-s1-e2": [
    {title:"1. Traducció de l'enunciat", prompt:"Escriu la condició d'accés en llenguatge lògic.", hint:"En horari laboral n'hi ha prou amb un sistema; fora d'horari en calen dos.", solution:"Variables: h = horari laboral, c = clau vàlida, t = targeta vàlida, e = empremta vàlida.\nSi h = 1: accés si c + t + e.\nSi h = 0: accés si almenys dos sistemes són vàlids: c·t + c·e + t·e.", keywords:["horari","clau","targeta","empremta"]},
    {title:"2. Funció lògica simplificada", prompt:"Escriu la funció de sortida a.", hint:"Combina el cas laboral i el cas no laboral.", solution:"a = h·(c + t + e) + ¬h·(c·t + c·e + t·e).\nAquesta expressió ja és clara i adequada per dibuixar amb portes AND, OR i NOT.", keywords:["h","c","t","e","AND","OR"]},
    {title:"3. Diagrama de portes", prompt:"Descriu el circuit lògic equivalent.", hint:"Primer crea els productes i després suma'ls amb OR.", solution:"Cal una NOT per obtenir ¬h. Després es construeix h·(c+t+e) i també ¬h·(ct+ce+te). Finalment, una OR uneix els dos blocs i dona la sortida a.", keywords:["NOT","AND","OR"]}
  ],
  "full-tec2024-s1-e3": [
    {title:"a) Potència elèctrica consumida", prompt:"Calcula la potència necessària per pujar a velocitat constant.", hint:"La força útil contra el pendent és m·g·sin(α).", solution:"v = 8 km/h = 2,222 m/s.\nFpendent = m·g·sinα = 70·9,81·sin7° = 83,7 N.\nPmec = F·v = 83,7·2,222 = 186 W.\nPelèc = Pmec/η = 186/0,85 = 219 W.\nResultat: Pelèc ≈ 219 W.", keywords:["219","186","sin"]},
    {title:"b) Velocitat angular i parell", prompt:"Calcula ω del motor i el parell que subministra.", hint:"La roda va connectada directament a l'eix del motor.", solution:"Radi de la roda: r = 0,140/2 = 0,070 m.\nω = v/r = 2,222/0,070 = 31,75 rad/s.\nΓ = Pmec/ω = 186/31,75 = 5,86 N·m.\nResultat: ωmot ≈ 31,8 rad/s i Γ ≈ 5,9 N·m.", keywords:["31,8","5,86","parell"]},
    {title:"c) Percentatge d'energia consumida", prompt:"Calcula l'energia elèctrica gastada i compara-la amb la bateria.", hint:"El temps és distància/velocitat.", solution:"Temps: t = 2000/2,222 = 900 s = 0,25 h.\nEcons = Pelèc·t = 219 W·0,25 h = 54,7 Wh.\nPercentatge = 54,7/250·100 = 21,9 %.\nResultat: es consumeix aproximadament el 22 % de la bateria.", keywords:["54,7","21,9","22"]}
  ],
  "full-tec2024-s1-e4": [
    {title:"a) Treball mecànic", prompt:"Calcula l'energia cinètica final del cotxe.", hint:"Parteix del repòs, per tant només cal l'energia cinètica final.", solution:"v = 50 km/h = 13,89 m/s.\nW = ΔEc = 1/2·m·v² = 0,5·1250·13,89² = 120563 J.\nResultat: W ≈ 1,21·10⁵ J = 120,6 kJ.", keywords:["120563","120,6","cinètica"]},
    {title:"b) Benzina utilitzada", prompt:"Aplica el rendiment del motor i el poder calorífic.", hint:"L'energia del combustible és més gran que el treball mecànic.", solution:"Ecomb = W/η = 120563/0,25 = 482253 J.\npc = 46 MJ/kg = 46·10⁶ J/kg.\nmbenz = Ecomb/pc = 482253/(46·10⁶) = 0,0105 kg = 10,5 g.", keywords:["482253","0,0105","10,5"]},
    {title:"c) Petjada de CO₂", prompt:"Passa la massa de benzina a volum i aplica el factor d'emissió.", hint:"ρ = 0,72 kg/L perquè 0,72 g/cm³ equival a 0,72 kg/L.", solution:"Vbenz = m/ρ = 0,0105/0,72 = 0,0146 L.\nmCO₂ = V·FE = 0,0146·2,157 = 0,0314 kg = 31,4 g de CO₂.\nResultat: petjada aproximada de 31 g de CO₂.", keywords:["0,0146","31,4","CO2"]}
  ],
  "full-tec2024-s1-e5": [
    {title:"a) Diagrama de cos lliure", prompt:"Identifica les forces que actuen sobre el peu.", hint:"Hi ha pes, força muscular i reaccions a l'articulació.", solution:"Forces: pes P = m·g aplicat al centre d'inèrcia G; força muscular Fm aplicada segons la línia d'acció del tibial anterior; reaccions de l'articulació O amb components Ox i Oy. El peu està en equilibri estàtic: ΣF = 0 i ΣM_O = 0.", keywords:["pes","Fm","Ox","Oy","moment"]},
    {title:"b) Força del múscul", prompt:"Planteja l'equilibri de moments respecte d'O.", hint:"La reacció a O no fa moment si prens moments respecte d'O.", solution:"Procediment: ΣM_O = 0. El moment del múscul és Fm·a. El moment del pes és P·dG, on dG és el braç de moment del pes respecte d'O que cal llegir a la figura. Per tant, Fm = (m·g·dG)/a. Amb a = 30 mm = 0,030 m. Cal substituir dG amb la distància indicada a la figura original.", keywords:["moment","dG","0,030","figura"]},
    {title:"c) Forces a l'articulació", prompt:"Un cop coneguda Fm, aplica equilibri de forces.", hint:"Descompon Fm amb l'angle φ = 9° respecte de la vertical.", solution:"Components de la força muscular: Fmx = Fm·sinφ i Fmy = Fm·cosφ, segons el sentit de la línia d'acció. Equilibri: ΣFx = 0 → Ox compensa Fmx. ΣFy = 0 → Oy compensa el pes i la component vertical de Fm. Els signes depenen del sentit dibuixat a la figura.", keywords:["ΣFx","ΣFy","sin","cos"]}
  ],
  "full-tec2024-s1-e6": [
    {title:"a) Reactància inductiva", prompt:"Calcula XL de la bobina.", hint:"Passa 7 mH a 0,007 H.", solution:"XL = 2πfL = 2π·50·0,007 = 2,20 Ω.", keywords:["2,20","reactància"]},
    {title:"b) Factor de potència", prompt:"Calcula el cos φ de la càrrega RL sèrie.", hint:"En RL sèrie, cosφ = R/|Z|.", solution:"|Z| = √(R²+XL²)=√(5²+2,20²)=5,46 Ω.\ncosφ = R/|Z| = 5/5,46 = 0,915.", keywords:["5,46","0,915"]},
    {title:"c-d-e) Impedància, corrent i tensió", prompt:"Fes servir la potència activa mesurada pel wattímetre.", hint:"En una càrrega RL sèrie, la potència activa es dissipa només a R.", solution:"Z = 5 + j2,20 Ω, |Z| = 5,46 Ω.\nW = P = I²R → I = √(950/5) = 13,78 A.\nU = I·|Z| = 13,78·5,46 = 75,3 V.\nResultats: A1 ≈ 13,8 A i U1 ≈ 75 V.", keywords:["13,78","75,3","Z"]}
  ],
  "full-tec2024-s5-e3": [
    {title:"a) Energia del sac", prompt:"Calcula l'energia química disponible en un sac de pèl·lets.", hint:"Multiplica massa pel poder calorífic.", solution:"Econs = m·pc = 15·5,23 = 78,45 kWh.", keywords:["78,45"]},
    {title:"b) Energia útil i cost", prompt:"Aplica el rendiment i calcula el cost per kWh útil.", hint:"El cost per kWh s'ha de calcular amb l'energia útil, no amb la química.", solution:"Eútil = η·Econs = 0,89·78,45 = 69,82 kWh.\nCost per kWh útil = 6 €/69,82 kWh = 0,086 €/kWh.", keywords:["69,82","0,086"]},
    {title:"c-d) Durada i cendra", prompt:"Calcula hores de funcionament i residu generat.", hint:"La cendra és el 0,7% de la massa inicial.", solution:"t = 15 kg / 1,483 kg/h = 10,1 h.\nmcendra = 0,007·15 = 0,105 kg = 105 g.\nResultats: unes 10,1 h i 105 g de cendra.", keywords:["10,1","0,105","105"]}
  ],
  "full-tec2024-s5-e4": [
    {title:"a) Relació de transmissió", prompt:"Calcula la relació total del tren d'engranatges.", hint:"Multiplica les relacions de cada etapa motriu/conduïda.", solution:"τ = ωroda/ωmotor = (z1/z2)·(z3/z4)·(z5/z6) = (23/57)·(16/33)·(18/38) = 0,0927.\nLa roda gira molt més lenta que el motor.", keywords:["0,0927","relació"]},
    {title:"b) Velocitat d'avanç", prompt:"Passa la velocitat de la roda a velocitat lineal.", hint:"Primer calcula nroda i després ωroda.", solution:"nroda = 4000·0,0927 = 370,7 min⁻¹.\nωroda = 2π·370,7/60 = 38,82 rad/s.\nv = ω·r = 38,82·0,30 = 11,65 m/s = 41,9 km/h.", keywords:["370","38,82","11,65","41,9"]},
    {title:"c) Parell a la roda", prompt:"Aplica rendiment i potència a la roda.", hint:"Psortida = η·Pmotor.", solution:"Psortida = 0,9·15000 = 13500 W.\nΓroda = P/ω = 13500/38,82 = 348 N·m.\nResultat: parell aproximat a la roda de 3,5·10² N·m.", keywords:["13500","348"]}
  ],
  "full-tec2024-s5-e5": [
    {title:"a) Resistència del fil", prompt:"Calcula la resistència del nicrom.", hint:"Primer calcula la secció del fil.", solution:"A = πd²/4 = π·(0,0008)²/4 = 5,03·10⁻⁷ m².\nR = ρL/A = 1,28·10⁻⁶·5,54 / 5,03·10⁻⁷ = 14,11 Ω.", keywords:["14,11","secció"]},
    {title:"b-c) Potència útil i intensitat", prompt:"Calcula potència absorbida, útil i intensitat.", hint:"La potència absorbida és U²/R.", solution:"Pabs = U²/R = 230²/14,11 = 3750 W.\nPútil = η·Pabs = 0,9·3750 = 3375 W.\nI = U/R = 230/14,11 = 16,3 A.", keywords:["3750","3375","16,3"]},
    {title:"d) Energia consumida", prompt:"Calcula l'energia durant 30 min.", hint:"30 min = 0,5 h.", solution:"Econs = Pabs·t = 3,75 kW·0,5 h = 1,875 kWh.\nEquivalent en joules: 1,875·3,6 MJ = 6,75 MJ.", keywords:["1,875","6,75"]}
  ],
  "full-tec2024-s5-e6": [
    {title:"a) Energia consumida per dutxa", prompt:"Calcula la calor útil de l'aigua i l'energia del gas.", hint:"75 L d'aigua equivalen a 75 kg.", solution:"Qútil = m·ce·ΔT = 75·4,18·20 = 6270 kJ = 6,27 MJ.\nEcons = Qútil/η = 6,27/0,87 = 7,21 MJ per dutxa.", keywords:["6,27","7,21"]},
    {title:"b) Nombre de dutxes", prompt:"Calcula quanta massa de butà es gasta per dutxa.", hint:"mgas = Econs/pc.", solution:"mgas = 7,21/47,7 = 0,151 kg per dutxa.\nn = 12,5/0,151 = 82,7 dutxes.\nResultat: aproximadament 82 dutxes completes.", keywords:["0,151","82"]},
    {title:"c) Cost d'una dutxa", prompt:"Suma el cost del butà i de l'aigua.", hint:"75 L = 0,075 m³.", solution:"Cost butà per dutxa = 17,66/82,7 = 0,214 €.\nCost aigua = 0,075·0,93 = 0,070 €.\nCost total ≈ 0,284 € per dutxa.", keywords:["0,214","0,070","0,284"]}
  ]
};
function applyPauSolutionsV12(){
  pauBank.forEach(x => {
    const sol = pauSolutionsV12[x.id] || pauSolutionsV12[String(x.id||'').replace(/^tec/, 'full-tec')];
    if(sol){
      x.resolucioPassos = sol;
      x.resolucio = sol.map(s => `${s.title}\n${s.solution}`).join('\n\n');
      x.estatResolucio = 'v12 resolt pas a pas';
    } else if(Number(x.any) <= 2023){
      x.estatResolucio = 'pendent de resolució completa en fase següent';
    }
  });
}
...
function renderPauBank(){
  ...
  if(!el) return;
  el.innerHTML = items.map(x => `<article class="card"><p><span class="pill">${esc(x.any)}</span> <span class="pill">${esc(x.materia)}</span> <span class="pill">${esc(x.serie)}</span></p><h3>${esc(x.exercici)} · ${esc(x.titol)}</h3><p><b>${esc(x.bloc)}</b> · ${esc(x.tipus)} · ${esc(x.nivell)}</p><p>${esc(x.resum)}</p>${x.estatResolucio?`<p><span class="pill">${esc(x.estatResolucio)}</span></p>`:''}<div class="btnrow"><button class="primary" onclick="openPauItem('${x.id}')">Treballar per parts</button>${x.enllac?`<button class="secondary" onclick="openExercise('${x.enllac}')">Obrir calculadora associada</button>`:''}</div></article>`).join('') || '<div class="notice">No hi ha fitxes amb aquest filtre.</div>';
}
...
function makePauSteps(x){
  ...
  if (Array.isArray(x.resolucioPassos) && x.resolucioPassos.length) {
    return x.resolucioPassos.map((st, idx) => ({
      title: st.title || `Pas ${idx+1}`,
      prompt: st.prompt || 'Resol aquest pas de manera raonada.',
      hint: st.hint || 'Revisa dades, unitats i fórmula abans de calcular.',
      solution: st.solution || '',
      keywords: st.keywords || String(st.solution||'').toLowerCase().split(/[^a-zà-ú0-9]+/).filter(w=>w.length>3).slice(0,12)
    }));
  }
  return [
...
function qualityPanelV13(){
  ...
  return `<section class="card quality-panel"><h2>Control de qualitat v14</h2><p>La v14 conserva el control de qualitat i afegeix mode examen, temporitzador i dossiers imprimibles.</p><div class="quality-grid"><div><b>${counts.calculator||0}</b><span>amb calculadora</span></div><div><b>${counts.guidedFigure||0}</b><span>guiats amb figura</span></div><div><b>${counts.textOnly||0}</b><span>resolts textualment</span></div><div><b>${counts.review||0}</b><span>pendent revisió</span></div></div><div class="progressbar"><div style="width:${pct((counts.calculator||0)+(counts.guidedFigure||0)+(counts.textOnly||0))}%"></div></div><p class="small">Percentatge de fitxes amb resolució o guia docent: ${pct((counts.calculator||0)+(counts.guidedFigure||0)+(counts.textOnly||0))}%.</p></section>`;
}
...
renderPauBank = function(){
  ...
  const stats = document.getElementById('pauBankStats');
  if(stats) stats.innerHTML = `<p><b>${items.length}</b> fitxes mostrades de <b>${data.length}</b>. Filtres v14: any, sèrie, tema, mode i estat de verificació.</p>`;
  const el = document.getElementById('pauBankCards');
  if(!el) return;
...
renderPauHeader = function(x){
  return `<h2>${esc(x.exercici)} · ${esc(x.titol)}</h2><p><span class="pill">${esc(x.any)}</span> <span class="pill">${esc(x.materia)}</span> <span class="pill">${esc(x.serie)}</span> <span class="pill">${esc(x.bloc)}</span> <span class="pill">${esc(x.nivell)}</span> ${statusBadgeV13(x)}</p><p>${esc(x.resum)}</p><div class="notice verify-note"><b>Estat de verificació:</b> ${esc(x.verificacioNota || inferVerificationV13(x).note)}</div><details class="enunciat-box" open><summary>Enunciat de treball</summary><p>${esc(x.enunciat || x.resum)}</p></details>${renderPauFigures(x)}`;
};
...
printClassSheet = function(id, withSolutions=false){
  ...
  const blanks = `<div class="blank-lines"></div>`;
  const rubric = x.criterisCorreccioV13 || buildRubricV13(x);
  const sheet = `<section class="card print-sheet"><h2>${withSolutions ? 'Fitxa PAU amb solucions v14' : 'Fitxa de treball PAU v14'}</h2>${renderPauHeader(x)}<p><b>Temps recomanat:</b> 20-35 min segons dificultat.</p>${withSolutions?`<div class="notice"><b>Estat de verificació:</b> ${esc(x.verificacio || inferVerificationV13(x).label)}. ${esc(x.verificacioNota || inferVerificationV13(x).note)}</div>`:''}${steps.map((st,i)=>`<article class="sheet-step"><h3>${esc(st.title)}</h3><p>${esc(st.prompt)}</p>${withSolutions?`<div class="ok"><b>Solució orientativa:</b><pre>${esc(st.solution)}</pre></div>`:blanks}</article>`).join('')}${withSolutions?`<h3>Criteris de correcció</h3><table><tbody>${rubric.map(r=>`<tr><td><b>${esc(r[0])}</b></td><td>${esc(r[1])}</td></tr>`).join('')}</tbody></table>`:''}<div class="no-print btnrow"><button class="secondary" onclick="openPauItem('${id}')">Tornar a l’activitat</button><button class="primary" onclick="window.print()">Imprimir ara</button></div></section>`;
  app.innerHTML = sheet;
  window.scrollTo(0, 0);
};
...
/* === v14 · Mode examen PAU i dossiers imprimibles === */
const V14_VERSION = 'v14 · mode examen i dossiers imprimibles';
let v14ExamItems = [];
let v14ExamStartedAt = null;
let v14TimerHandle = null;

function uniqueV14(field){
  return [...new Set(pauData().map(x => x[field]).filter(Boolean))].sort((a,b)=>String(a).localeCompare(String(b),'ca',{numeric:true}));
}
function examFilterOptionsV14(){
  const years = uniqueV14('any').sort((a,b)=>b-a).map(y=>`<option>${y}</option>`).join('');
  const series = uniqueV14('serie').map(s=>`<option>${esc(s)}</option>`).join('');
  const mats = uniqueV14('materia').map(m=>`<option>${esc(m)}</option>`).join('');
  return {years, series, mats};
}
function getFilteredItemsV14(prefix='exam'){
  const materia = document.getElementById(prefix+'Materia')?.value || '';
  const year = document.getElementById(prefix+'Year')?.value || '';
  const serie = document.getElementById(prefix+'Serie')?.value || '';
  const q = (document.getElementById(prefix+'Search')?.value || '').toLowerCase().trim();
  const verify = document.getElementById(prefix+'Verify')?.value || '';
  let items = pauData().filter(x => (!materia || x.materia === materia) && (!year || String(x.any) === String(year)) && (!serie || x.serie === serie));
  if(verify) items = items.filter(x => inferVerificationV13(x).key === verify);
  if(q){
    items = items.filter(x => `${x.any} ${x.materia} ${x.serie} ${x.exercici} ${x.bloc} ${x.titol} ${x.resum} ${x.enunciat} ${x.tipus}`.toLowerCase().includes(q));
  }
  return items;
}
function shuffleV14(arr){
  const a = [...arr];
  for(let i=a.length-1;i>0;i--){ const j=Math.floor(Math.random()*(i+1)); [a[i],a[j]]=[a[j],a[i]]; }
  return a;
}
function minutesV14(){ return Math.max(5, parseInt(document.getElementById('examMinutes')?.value || '90', 10) || 90); }
function renderTimerV14(){
  const el = document.getElementById('examTimer');
  if(!el || !v14ExamStartedAt) return;
  const total = minutesV14()*60;
  const used = Math.floor((Date.now()-v14ExamStartedAt)/1000);
  const left = Math.max(0, total-used);
  const mm = String(Math.floor(left/60)).padStart(2,'0');
  const ss = String(left%60).padStart(2,'0');
  el.textContent = `${mm}:${ss}`;
  el.className = left < 300 ? 'pill bad' : 'pill';
}
function startTimerV14(){
  v14ExamStartedAt = Date.now();
  if(v14TimerHandle) clearInterval(v14TimerHandle);
  v14TimerHandle = setInterval(renderTimerV14, 1000);
  renderTimerV14();
}
function stopTimerV14(){ if(v14TimerHandle) clearInterval(v14TimerHandle); v14TimerHandle=null; v14ExamStartedAt=null; renderTimerV14(); }

function examenV14(){
  const opts = examFilterOptionsV14();
  app.innerHTML = `<section class="card"><h2>Mode examen PAU · v14</h2><p>Genera una prova a partir del banc PAU multiany, amb temporitzador, full imprimible, versió amb solucions i informe de revisió.</p><div class="grid"><div class="field"><label>Matèria</label><select id="examMateria"><option value="">Totes</option>${opts.mats}</select></div><div class="field"><label>Any</label><select id="examYear"><option value="">Tots</option>${opts.years}</select></div><div class="field"><label>Sèrie</label><select id="examSerie"><option value="">Totes</option>${opts.series}</select></div><div class="field"><label>Tema o paraula clau</label><input id="examSearch" placeholder="Ex.: motor, energia, lògica, trifàsica..."></div><div class="field"><label>Estat de verificació</label><select id="examVerify"><option value="">Tots</option><option value="calculator">Automatitzable</option><option value="guidedFigure">Guiat amb figura</option><option value="textOnly">Resolt textualment</option><option value="review">Revisió pendent</option></select></div><div class="field"><label>Nombre d’exercicis</label><input id="examCount" type="number" min="1" max="8" value="4"></div><div class="field"><label>Temps recomanat</label><input id="examMinutes" type="number" min="5" value="90"></div><div class="field"><label>Selecció</label><select id="examRandom"><option value="yes">Aleatòria</option><option value="no">Primers resultats del filtre</option></select></div></div><div class="btnrow no-print"><button class="primary" onclick="generateExamV14()">Generar examen</button><button class="secondary" onclick="startTimerV14()">Iniciar temporitzador</button><button class="secondary" onclick="stopTimerV14()">Aturar</button><span id="examTimer" class="pill">--:--</span></div><div id="examArea"></div></section><section class="card"><h2>Dossiers imprimibles · v14</h2><p>Crea un dossier per tema, any o sèrie. Pots imprimir-lo com a dossier d’alumne o dossier docent amb solucions.</p><div class="grid"><div class="field"><label>Matèria</label><select id="dossierMateria"><option value="">Totes</option>${opts.mats}</select></div><div class="field"><label>Any</label><select id="dossierYear"><option value="">Tots</option>${opts.years}</select></div><div class="field"><label>Sèrie</label><select id="dossierSerie"><option value="">Totes</option>${opts.series}</select></div><div class="field"><label>Tema o paraula clau</label><input id="dossierSearch" placeholder="Ex.: energia, mecanismes, circuits..."></div><div class="field"><label>Estat de verificació</label><select id="dossierVerify"><option value="">Tots</option><option value="calculator">Automatitzable</option><option value="guidedFigure">Guiat amb figura</option><option value="textOnly">Resolt textualment</option><option value="review">Revisió pendent</option></select></div><div class="field"><label>Màxim de fitxes</label><input id="dossierCount" type="number" min="1" max="52" value="12"></div></div><div class="btnrow no-print"><button class="primary" onclick="generateDossierV14(false)">Crear dossier alumne</button><button class="secondary" onclick="generateDossierV14(true)">Crear dossier docent</button></div><div id="dossierArea"></div></section>`;
}

function buildExamSheetV14(items, withSolutions=false, title='Prova PAU generada'){
  const date = new Date().toLocaleDateString('ca-ES');
  return `<section class="card print-sheet exam-sheet"><h2>${esc(title)}</h2><p><b>Data:</b> ${date} · <b>Nombre d’exercicis:</b> ${items.length} · <b>Mode:</b> ${withSolutions?'docent amb solucions':'alumne'}</p><p class="notice no-print"><b>Ús recomanat:</b> imprimeix la versió alumne per fer la prova i la versió docent per corregir. Les fitxes amb figura inclouen les pàgines originals integrades.</p>${items.map((x,idx)=>`<article class="exam-item"><h3>${idx+1}. ${esc(x.any)} · ${esc(x.serie)} · ${esc(x.exercici)} · ${esc(x.titol)}</h3><p><span class="pill">${esc(x.materia)}</span> <span class="pill">${esc(x.bloc)}</span> <span class="pill">${esc(x.tipus)}</span> ${statusBadgeV13(x)}</p><div class="enunciat-box"><b>Enunciat</b><p>${esc(x.enunciat || x.resum)}</p></div>${(x.apartats||[]).length?`<h4>Apartats</h4><ul>${x.apartats.map(a=>`<li>${esc(a)}</li>`).join('')}</ul>`:''}${renderPauFigures(x)}${withSolutions?`<details class="explain" open><summary>Solució orientativa</summary><pre>${esc(x.resolucio || buildFinalAnswerV13(x))}</pre></details><details class="explain" open><summary>Criteris de correcció</summary><table><tbody>${buildRubricV13(x).map(r=>`<tr><td><b>${esc(r[0])}</b></td><td>${esc(r[1])}</td></tr>`).join('')}</tbody></table></details>`:`<h4>Resposta</h4><div class="blank-lines"></div><h4>Esquema o càlculs</h4><div class="blank-lines"></div>`}</article>`).join('')}<div class="no-print btnrow"><button class="secondary" onclick="setView('examen')">Tornar al mode examen</button><button class="primary" onclick="window.print()">Imprimir ara</button></div></section>`;
}

function generateExamV14(){
  let items = getFilteredItemsV14('exam');
  const count = Math.max(1, Math.min(8, parseInt(document.getElementById('examCount')?.value || '4',10) || 4));
  if(document.getElementById('examRandom')?.value === 'yes') items = shuffleV14(items);
  v14ExamItems = items.slice(0, count);
  const el = document.getElementById('examArea');
  if(!el) return;
  if(!v14ExamItems.length){ el.innerHTML = '<div class="notice">No hi ha exercicis amb aquest filtre.</div>'; return; }
  el.innerHTML = `<div class="ok"><b>Examen generat:</b> ${v14ExamItems.length} exercicis seleccionats.</div><div class="btnrow no-print"><button class="primary" onclick="printExamV14(false)">Imprimir examen alumne</button><button class="secondary" onclick="printExamV14(true)">Imprimir amb solucions</button><button class="secondary" onclick="renderExamReviewV14()">Veure informe de revisió</button></div><div class="grid">${v14ExamItems.map(x=>`<article class="card"><h3>${esc(x.exercici)} · ${esc(x.titol)}</h3><p><span class="pill">${esc(x.any)}</span> <span class="pill">${esc(x.serie)}</span> ${statusBadgeV13(x)}</p><p>${esc(x.resum)}</p><button class="secondary" onclick="openPauItem('${x.id}')">Obrir fitxa</button></article>`).join('')}</div>`;
}
function printExamV14(withSolutions=false){
  if(!v14ExamItems.length) generateExamV14();
  if(!v14ExamItems.length) return;
  app.innerHTML = buildExamSheetV14(v14ExamItems, withSolutions, withSolutions?'Prova PAU generada · solucionari docent':'Prova PAU generada · full alumne');
  window.scrollTo(0,0);
}
function renderExamReviewV14(){
  const el = document.getElementById('examArea'); if(!el) return;
  if(!v14ExamItems.length){ el.innerHTML = '<div class="notice">Primer genera un examen.</div>'; return; }
  const byStatus = v14ExamItems.reduce((a,x)=>{ const k=inferVerificationV13(x).label; a[k]=(a[k]||0)+1; return a; },{});
  const rows = Object.entries(byStatus).map(([k,v])=>`<tr><td>${esc(k)}</td><td>${v}</td></tr>`).join('');
  el.innerHTML = `<section class="card"><h3>Informe de revisió de la prova</h3><table><tbody><tr><td>Exercicis</td><td>${v14ExamItems.length}</td></tr>${rows}</tbody></table><h4>Temes inclosos</h4><ul>${v14ExamItems.map(x=>`<li>${esc(x.any)} · ${esc(x.bloc)} · ${esc(x.titol)}</li>`).join('')}</ul><p class="small">Per avaluació formal, prioritza exercicis automatitzables, resolts textualment o revisats manualment.</p></section>`;
}
function generateDossierV14(withSolutions=false){
  let items = getFilteredItemsV14('dossier');
  const count = Math.max(1, Math.min(52, parseInt(document.getElementById('dossierCount')?.value || '12',10) || 12));
  items = items.slice(0,count);
  const el = document.getElementById('dossierArea'); if(!el) return;
  if(!items.length){ el.innerHTML = '<div class="notice">No hi ha fitxes amb aquest filtre.</div>'; return; }
  el.innerHTML = `<div class="ok"><b>Dossier creat:</b> ${items.length} fitxes seleccionades.</div><div class="btnrow no-print"><button class="primary" onclick="printDossierV14(${withSolutions})">Imprimir dossier ${withSolutions?'docent':'alumne'}</button><button class="secondary" onclick="saveCurrent('Dossier PAU v14', document.getElementById('dossierArea').innerText)">Guardar resum</button></div><div class="grid">${items.map(x=>`<article class="card"><h3>${esc(x.exercici)} · ${esc(x.titol)}</h3><p><span class="pill">${esc(x.any)}</span> <span class="pill">${esc(x.serie)}</span> <span class="pill">${esc(x.bloc)}</span></p><p>${esc(x.resum)}</p></article>`).join('')}</div>`;
  v14ExamItems = items;
}
function printDossierV14(withSolutions=false){
  if(!v14ExamItems.length) return;
  app.innerHTML = buildExamSheetV14(v14ExamItems, withSolutions, withSolutions?'Dossier PAU docent amb solucions':'Dossier PAU alumne');
  window.scrollTo(0,0);
}

const oldSetViewV14 = setView;
setView = function(view){
  const views = {inici, pau, exercicis: exercicisView, calculadores, practica, formulari, historial, examen: examenV14};
  (views[view] || inici)();
  $$('.tab').forEach(b => b.classList.toggle('active', b.dataset.view === view));
  window.scrollTo({top:0, behavior:'smooth'});
};

const oldIniciV14 = inici;
inici = function(){
  oldIniciV14();
  const first = app.querySelector('.card');
  if(first){
    first.insertAdjacentHTML('beforeend', `<div class="notice"><b>Novetat v14:</b> mode examen PAU i dossiers imprimibles per any, tema o estat de verificació.</div><div class="btnrow"><button class="primary" onclick="setView('examen')">Obrir mode examen</button><button class="secondary" onclick="setView('pau')">Explorar banc PAU</button></div>`);
  }
};
